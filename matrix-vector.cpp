    ...
    auto vector_test = [&]() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                // ymm_result = _mm256_fmadd_ps(ymm_cols[j], ymm_v, ymm_result);
            }
    ...
    */
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < 8; j++) {
            ...
            assert(std::abs(results_seq[i][j] - results_vec[i][j]) <= 1E-6);
            /*
            if (results_seq[i][j] - results_vec[i][j] != 0) {
                std::cout << "diff: " << (results_seq[i][j] - results_vec[i][j]) << std::endl;
            }
            */
        }
        // print_vector(diff, 4);
